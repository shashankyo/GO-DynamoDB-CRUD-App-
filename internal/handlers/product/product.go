...
func (h *Handler) Put(w http.ResponseWriter, r *http.Request) {
	...
	producBody, err := h.getBodyAndValidate(r,ID)
...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	ID, err := uuid.Prse(chi.URLParam(r,"ID"))
	if err != nil {
		HttpStatus.StatusBadRequest(w,r,errors.New("ID is not uuid valid"))
		return 
	} 
	h.Controller.Remove(ID); err != nil {
		HttpStatus.StatusInternalServerError(w,r,err)
		return 
	}
	HttpStatus.StatusContent(w,r )
}
...
func (h *Handler) getBodyAndValidate(r *http.Request, ID uuid.UUID)(*EntityProduct.Product, error ) {
	...
	body,err := ConvertIoReaderToStruct(r.Body,productBody)
	if err != nil {
		return &EntityProduct{}, errors.New("body is required")
	}
...
func setDefaultValues(product *EntityProduct, ID uuid.UUID){
